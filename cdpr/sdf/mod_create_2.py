...
def ConfigFromPkg(pkg, path):
    ...
    rospack.list() 
    xacro_path = rospack.get_path(pkg) + '/' + path + '/'
    cfg_dict = {}
    # build dictionary
    for cfg_file in listdir(xacro_path):
        if 'cfg_' in cfg_file and '.yml' in cfg_file:
            cfg_name = cfg_file.replace('cfg_', '')[:-4]
            with open(xacro_path + cfg_file) as f:
                cfg_dict[cfg_name] = yaml.load(f)
    return DictsToNamespace(cfg_dict), xacro_path
...
def WriteXACRO(xacro, filename):
    etree.indent(xacro)
    #xacro_content = '<?xml version="1.0"?>\n' + str(etree.tostring(sdf, pretty_print=True))

    xacro_content = minidom.parseString(etree.tostring(xacro,pretty_print=True)).toprettyxml()
    print(xacro_content)
    with open(filename, 'w') as f:
        f.write(xacro_content)
        rospy.logdebug("printing")
