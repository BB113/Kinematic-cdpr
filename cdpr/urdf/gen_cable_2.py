...
if __name__ == '__main__':
    ...
    if sim_cables:
        ...
        for i, cbl in enumerate(config.points):
            ...
            print("position cable "+str(l_xyz))
            xyz_c = str(l_xyz[0]+" "+l_xyz[1]+" "+l_xyz[2])
            rpy= str(l_xyz[3]+" "+l_xyz[4]+" "+l_xyz[5])

            origin=etree.SubElement(collision, 'origin', xyz = xyz_c, rpy = rpy)
            ...
            origin=etree.SubElement(inertial, 'origin', xyz = xyz_c, rpy = rpy)
 ...
 # virtual link around X
            ...
            visual = etree.SubElement(link, 'visual')
            origin=etree.SubElement(inertial, 'origin', xyz = xyz, rpy = rpy)
            geometry=etree.SubElement(visual, 'geometry')
            ...
            origin=etree.SubElement(inertial, 'origin', xyz = xyz_vlx, rpy = rot_vlx)
            mass=etree.SubElement(inertial, 'mass', value = "0.001")
            inertia=etree.SubElement(inertial, 'inertia', ixx="0.001", ixy="0", ixz="0", iyy="0.001", iyz="0", izz="0.001")
            visual = etree.SubElement(link, 'visual')
            origin=etree.SubElement(inertial, 'origin', xyz = xyz_vlx, rpy = rot_vlx)
            ...
            origin_xpf=etree.SubElement(joint, 'origin', xyz = xyz_rjx, rpy = rot_rjx)
